...
int _EncodeWAVEFileToAMRFile(const char* pchWAVEFilename, const char* pchAMRFileName, int nChannels, int nBitsPerSample)
{
	FILE* fpwave;
	FILE* fpamr;
	/* input speech vector */
	...
	size_t bytes = fwrite(AMR_MAGIC_NUMBER, sizeof(char), strlen(AMR_MAGIC_NUMBER), fpamr);
...
// 如果用户确定取消了录音，就一直读取数据知道size 值为0。PS：最后的size可能不够一个buffer，但是依然要处理
	while (1) {
        size_t size = ReadPCMFrame(speech, fpwave, nChannels, nBitsPerSample);
        /*
        if (isRecord) {
            if (size < 160) {
...
// 将AMR文件解码成WAVE文件
int _DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
